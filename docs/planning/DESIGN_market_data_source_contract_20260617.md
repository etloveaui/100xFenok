...
- `scripts/audit-market-data.py`
  ...
  - reports `incremental_etf.status` (`waiting|warn|pass|fail`) from
    ...
    optional `backfill/incremental_plan_latest.json`, and
    `market_facts.coverage.stockanalysis_yf_fallback` so scheduled self-heal
    runs have a durable post-run proof surface while staged history refreshes
    can publish a no-network preflight plan separately from completed-run
    evidence;
