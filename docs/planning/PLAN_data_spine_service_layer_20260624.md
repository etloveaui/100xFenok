...
- [x] P15-0/P9-G: restore `/multichart` as the existing stock compare route. It removes the `/macro-chart` redirect, embeds `/tools/asset/multichart.html`, restores Stooq daily CSV through the owner-owned Worker proxy, keeps only a 24h browser localStorage cache, and preserves `/macro-chart` as the native macro workbench.
- [x] P15-Fusion: Stooq stock/ETF/index series are now a delimiter-safe source type inside `/macro-chart`, so market symbols and macro series compare in one chart. Canonical contract: `CONTRACT_macro_chart_stooq_fusion_20260624.md`. `/multichart` stays as a known stock-compare address under the fused chart surface.
- [x] P16 IA/design-system thin slice: `/explore` is labeled `워크벤치`; `/macro-chart` is promoted to the official `차트` AppShell rail/mobile entry; `/multichart` shares the chart active state; header Analytics remains narrow as Radar, Insights, and `워크벤치`; a minimal additive `Surface` primitive starts token-based reuse.
...
- Macro smoke: `/macro-chart`, official `차트` rail/mobile entry, restored `/multichart` stock compare frame under chart active state, full CSV export, PNG export, mobile picker, mobile formula/status chips, share URL `range` + hidden-series + keyed axis + formula round trip, saved user preset apply, corrupted saved hidden-state handling, range zoom controls, analysis lenses, analysis summary, connection links, static macro catalog lenses/surfaces, Workbench(`/explore`) playbook/ETF entry links, narrow header `워크벤치` entry, Stooq Worker proxy smoke, and Stooq+macro fusion route smoke (`stq~NVDA.US` + `M2SL`).
