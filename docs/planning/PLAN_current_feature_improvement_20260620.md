...
- F2 (M, $-LLM) — ✅ 10-K/10-Q batch landed: summary auto-gen pipeline (feno-edgar extract → free LLM → artifact + manifest → qa gate) produced ready summaries for 39 10-K + 96 10-Q rows. 8-K Item 2.02 earnings target set is now 133 rows after SEC submissions refresh; tranche 1 landed 8 Gemini-free summaries with Exhibit 99.1 evidence extraction, numeric grounding, source/public parity, and no raw English scale words in user-facing summary text. Remaining F2 scope = continue 8-K earnings tranches, then separately design shorter event-note treatment for non-earnings 8-Ks. Free pool first.
