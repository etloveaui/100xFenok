...
- P5 SEO cleanup owns a public sitemap, live Worker-origin metadata base, stock/ETF/post canonical URLs, robots disallow for admin/live/travel aliases, and removal of the permanent `/travel/*` admin redirect. 2026-07-07 update: the `100xfenok` Cloudflare Pages project was deleted after legacy deployment purge; the Worker origin is the canonical app origin. The stale GitHub `github-pages` environment was also removed after the GitHub Pages site API returned 404. External check-suite auto-triggers are disabled for Cloudflare Workers and Pages and Claude; the GitHub Pages system app remains GitHub-owned and cannot be disabled through the repo admin API.
