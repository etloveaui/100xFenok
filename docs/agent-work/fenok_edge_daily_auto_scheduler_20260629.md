...
- `scripts/fetch-fenok-occ-options-volume.mjs --s0-occ-missing --batch-size 50 --batch-index 0 --date 20260626 --max-requests 100 --plan-only` selects only the current 108 active plain-US OCC gaps after the 2026-06-30 fifth bounded OCC slice. It is a plan-only command and does not fetch/write.
...
- Live OCC gap filling remains approval-gated. Current bounded budget is 108 tickers * 2 sides = 216 side requests: two 50-ticker batches at 100 side requests plus one 8-ticker final batch at 16 side requests.
