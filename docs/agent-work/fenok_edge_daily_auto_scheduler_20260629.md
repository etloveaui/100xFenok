...
- Current OCC missing 64 splits into 6 plain-US rows that still need explicit no-listed-options/policy evidence, 56 non-US suffix `US_CLASS` rows that need mapping/denominator policy, and 2 Berkshire class-share symbols that still need source/symbol-form resolution before being marked unavailable.
...
- The 20260626 narrow OCC partial follow-up resolved the 13 prior one-side loaded / one-side `no_record` rows into usable OCC rows. Only the 6 both-side `no_record` evidence-only plain-US rows remain in this class.
...
- Live OCC gap filling remains bounded and source-date guarded. The older 19-ticker / 38-side unresolved plain-US budget is now down to the 6 both-side `no_record` rows, which remain evidence-only unless a separate no-options proof path and owner-accepted policy are added.
