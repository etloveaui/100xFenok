...
| S3 ETF lane | PUBLIC surface, not DAILY/GATED | YF scheduled branch processes ETF history gaps through one rolling shard per run, capped at 140; StockAnalysis scheduled branch backfills up to 40 ETF details per run | 5,301 normalized ETF candidates; 4,484 eligible vanilla ETFs scored in the separate ETF lane after classification plus conservative heuristic exclusions; named ETF gate verifies the compact public summary mirror, ETF signal API route, and ETF detail UI card; generated admin readiness evidence is `data/admin/fenok-edge-etf-daily1y-readiness.json` with `4484 = 3366 complete + 584 fetchable + 534 inception-limited`; exact fetchable ticker plan is `data/admin/fenok-edge-etf-daily1y-fetchable-plan.json` with 584 tickers / 5 bounded batches at 120 each | `daily=false`, `gated=false`, `public_done_claim_allowed=false`; fetchable 1Y continuity gaps still block ETF paid-ready wording |
...
- Current OCC missing 77 splits into 19 attempted plain-US rows that still need narrow partial refetch or explicit no-listed-options evidence, 56 non-US suffix `US_CLASS` rows that need mapping/denominator policy, and 2 Berkshire class-share symbols that need OCC symbol-form testing before being marked unavailable.
...
- `scripts/fetch-fenok-occ-options-volume.mjs --s0-occ-missing --batch-size 25 --batch-index 0 --date 20260626 --max-requests 50 --plan-only` selected 25 active plain-US OCC gaps for the 2026-06-30 eighth bounded OCC slice. It added 6 usable OCC rows; 19 plain-US gaps remain unresolved as 6 both-side `no_record` attempts plus 13 one-side `partial_no_record_or_form_gap` attempts, with no permanent no-listed-options promotion.
...
- OCC accepted-form policy is explicit: future one-side `loaded` + one-side `no_record` rows may be represented as accepted-form rows with the `no_record` side set to zero volume, while both-side `no_record` stays evidence-only and does not clear `occ_full_us_source_ready` until a durable no-listed-options policy is owner-accepted.
- Narrow OCC partial follow-up should use `scripts/fetch-fenok-occ-options-volume.mjs --s0-occ-partial-missing --plan-only --date 20260626 --batch-size 100 --max-requests 100` first; it selects only the 13 current one-side loaded / one-side `no_record` rows and excludes the 6 both-side `no_record` evidence-only rows.
...
- Live OCC gap filling remains approval-gated. Current narrow partial follow-up budget is 13 tickers * 2 sides = 26 side requests. The older 19-ticker / 38-side budget includes the 6 both-side `no_record` rows, which remain evidence-only unless a separate no-options proof path and owner-accepted policy are added.
...
- Missing daily proof: source/proxy rows are explicitly `not_public_scoring`; KRX covers 338/338 Korea rows, FINRA metric-ready covers 579/579 active plain-US rows, OCC covers 560/637 US-bucket rows, and the latest bounded US run remains a diagnostic reference only. The 91 Asia ex-Taiwan rows are explicit exclusion evidence for the current 975-row Korea+US daily scope; they must move into a new HKEX/SSE/SZSE source lane before any all-1,066 daily/gated claim.
