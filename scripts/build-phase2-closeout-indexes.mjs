...
function actionFrom(stock, context) {
  ...
  if (memberships.length > 0) {
    score += memberships.length * 5;
    reasons.push(`${memberships.join("/")} 편입`);
  }
  if (maxWeight >= 2) {
    score += Math.min(18, maxWeight * 1.2);
    reasons.push(`지수 비중 ${maxWeight.toFixed(1)}%`);
  }

  const guru = context.guruHolders;
  if (finite(guru) && guru >= 5) {
    score += Math.min(24, guru * 1.6);
    reasons.push(`구루 ${guru}명 보유`);
  }

  const consensus = context.consensus;
  if (finite(consensus?.equity_score)) {
    score += consensus.equity_score * 16;
    if (consensus.equity_score >= 0.5) reasons.push(`13F 컨센서스 ${consensus.equity_score.toFixed(2)}`);
  }

  const conviction = context.conviction;
  if (conviction) {
    score += Math.min(18, conviction.count * 5 + (conviction.maxWeight ?? 0) * 100);
  ...
  if (bandPct !== null) {
    if (bandPct <= 0.25) {
      score += 10;
      reasons.push(`PER 밴드 하위 ${Math.round(bandPct * 100)}%`);
    } else if (bandPct >= 0.75) {
      score -= 5;
      reasons.push(`PER 밴드 상위 ${Math.round(bandPct * 100)}%`);
    }
  }
  ...
  if (ret12m !== null) {
    if (ret12m >= 0.25) {
      score += 8;
      reasons.push(`12M 모멘텀 ${(ret12m * 100).toFixed(1)}%`);
    } else if (ret12m <= -0.2) {
      score -= 6;
      reasons.push(`12M 약세 ${(ret12m * 100).toFixed(1)}%`);
    }
  }

  if (finite(stock.dividendYield) && stock.dividendYield >= 0.03) {
    score += 5;
    reasons.push(`배당 ${(stock.dividendYield * 100).toFixed(1)}%`);
  }
  ...
  let label = "관찰";
  let bucket = "watch";
  if ((finite(guru) && guru >= 8) || conviction) {
    label = "구루/13F 주목";
    bucket = "smart_money";
  } else if (bandPct !== null && bandPct <= 0.25 && ret12m !== null && ret12m > 0) {
    label = "저평가+모멘텀";
    bucket = "value_momentum";
  } else if (maxWeight >= 3 || memberships.length >= 2) {
    label = "지수 핵심";
    bucket = "index_core";
  } else if (finite(stock.dividendYield) && stock.dividendYield >= 0.03) {
    label = "배당 점검";
    bucket = "income";
  } else if (ret12m !== null && ret12m >= 0.25) {
    label = "모멘텀 리더";
    bucket = "momentum";
  }
  ...
  return {
    ...
    perBandPct: bandPct !== null ? round(bandPct, 4) : null,
    perBandLabel: perBandLabel(bandPct),
  };
}
  ...
  const actionRows = rows
    .map((stock) => {
      ...
      const context = {
        ...
        sectorSmartMoney: sectorSmartMoney.get(stock.sector) ?? null,
      };
      ...
      return {
        ...
        quality_flags: qualityFlags(stock, context),
