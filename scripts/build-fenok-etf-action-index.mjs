...
const FORMULA_VERSION = "fenok-etf-action-index-v0.1-preview";
...
export function buildEtfActionIndex({ sourcePayload = null, generatedAt = new Date() } = {}) {
  ...
  const totalSignals = signalKeys.length || 8;
  ...
  const sortedRows = [...rows].sort((a, b) => b.action_score - a.action_score || a.ticker.localeCompare(b.ticker));
...
export function validateEtfActionIndex(payload, sourcePayload = null) {
  ...
  for (const row of rows) {
    ...
    if (!finite(row.action_score) || row.action_score < 0 || row.action_score > 100) errors.push(`${row.ticker}: action_score out of range`);
    if (!finite(row.signal_score) || row.signal_score < 0 || row.signal_score > 100) errors.push(`${row.ticker}: signal_score out of range`);
