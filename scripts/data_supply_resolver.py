...
PRIMARY_PROVIDER = "stockanalysis"
FALLBACK_PROVIDER = "yahoo_finance"
PRIMARY_RECOVERY_REQUIRED = 3
...
def _fresh(row: Mapping[str, Any], decided: dt.datetime) -> bool:
    if row["validation_status"] != "valid":
        return False
    age = int((decided - _timestamp(row["source_as_of"])).total_seconds())
    return 0 <= age <= ETF_DETAIL_FRESH_TTL_HOURS * 3600
...
class DataSupplyResolver:
    ...
    def resolve_etf_detail(
        ...
        *,
        ...
        if any(row["domain"] != "etf_detail" or row["entity"] != entity for row in rows):
            raise SchemaError("resolver observation identity mismatch")
        latest: dict[str, dict[str, Any]] = {}
        for row in rows:
            if row["provider"] not in {PRIMARY_PROVIDER, FALLBACK_PROVIDER}:
        ...
        primary_fresh = primary is not None and _fresh(primary, decided)
        fallback_fresh = fallback is not None and _fresh(fallback, decided)
        ...
        if prior is not None and not primary_fresh and not fallback_fresh:
            if set(latest) != {PRIMARY_PROVIDER, FALLBACK_PROVIDER}:
            ...
            if lkg_age > ETF_DETAIL_EMERGENCY_LKG_TTL_DAYS * 86400:
                ...
                return self.store.commit_prepared("etf_detail", transaction_id)
        ...
        if prior is None:
            ...
            if primary_fresh:
                ...
                if next_recovery_count >= PRIMARY_RECOVERY_REQUIRED:
                    selected = self._selection(primary, decided_at=decided_at, primary=True)
