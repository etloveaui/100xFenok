...
function runConfigAndFixtureChecks() {
  ...
  assert.equal(DATA_SUPPLY_DETECTION_CONFIG.lanes.length, 21);
  assert.equal(DATA_SUPPLY_DETECTION_CONFIG.lanes.flatMap((item) => item.producer_members).length, 25);
  ...
  // process — the RED evidence is retained here, not in a session anecdote.
  {
    ...
    // shrinks while the hand-written lanes stay 21 -> identity check fails.
  ...
  assert.equal(new Set(attemptsFixture.attempts.map((row) => `${row.lane_id}:${row.member_id ?? "_"}`)).size, 25);
}
...
function runAttemptShardChecks(artifactRoot) {
  ...
  assert.equal(emptyReport.lanes.filter((row) => row.endpoint.reason === "workflow_unobserved").length, 21, "empty private root keeps all lanes explicitly unobserved");
...
function runAttemptChecks(artifactRoot) {
  ...
  assert.equal(missingScheduledReport.logical_lane_count, 21);
  assert.equal(missingScheduledReport.producer_member_count, 25);
  ...
  assert.equal(missingCompositeReport.logical_lane_count, 21);
  assert.equal(missingCompositeReport.producer_member_count, 25);
...
function runCompositeSourceFoldChecks() {
  ...
  assert.equal(missingTreasuryReport.producer_member_count, 25);
...
function runCliReproduction(artifactRoot) {
  ...
  const verifierSource = String.raw`
    ...
    if (report.lanes.length !== 21) throw new Error("logical denominator mismatch");
    ...
    if (Object.values(members).reduce((sum, value) => sum + value, 0) !== 25) throw new Error("member denominator mismatch");
...
function runCurrentRepositoryDryRun() {
  ...
  assert.equal(result.report.logical_lane_count, 21);
  assert.equal(result.report.producer_member_count, 25);
