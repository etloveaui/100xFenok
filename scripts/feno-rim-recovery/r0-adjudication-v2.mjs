// R0 v2 re-adjudication — option (b) amendment after red-team criteria review.
// Frozen criteria: data/computed/feno-rim-recovery/r0-criteria-v2.json
//   sha256 8b42c0d19da04c537531ef7afb8aaaefc1a1503c8b50ab2fc470590378a71a00 (commit c94a4ffb01)
// Model construction stays VERBATIM x2-cross-sectional.mjs (no model change).
// v2 inference: ESS-adjusted CI + Newey-West lag 11 primary; block battery sensitivity only.
...
const CRIT_PATH = "data/computed/feno-rim-recovery/r0-criteria-v2.json";
const FROZEN_SHA = "8b42c0d19da04c537531ef7afb8aaaefc1a1503c8b50ab2fc470590378a71a00";
const SEED = 20260807;

// ---------- integrity gates ----------
if (sha256(path.join(ROOT, CRIT_PATH)) !== FROZEN_SHA) { console.error("ABORT: v2 criteria changed since freeze"); process.exit(3); }
const crit = rj(CRIT_PATH);
const v1crit = rj("data/computed/feno-rim-recovery/r0-criteria.json");
for (const [p, sha] of Object.entries(v1crit.data_contract.sha256)) {
  if (!fs.existsSync(path.join(ROOT, p)) || sha256(path.join(ROOT, p)) !== sha) { console.error("ABORT: input hash mismatch", p); process.exit(3); } }
for (const [s, sha] of Object.entries(v1crit.data_contract.yf_unadjusted_sha256)) {
...
for (const p of crit.planned_result_paths) {
  if (fs.existsSync(path.join(ROOT, p))) { console.error("ABORT: v2 result path already exists (freeze violation):", p); process.exit(3); } }
...
function mbb(v, block, seed, reps = 10000) { if (v.length < 4) return null; const rnd = makeRng(seed);
  ...
  return { mean: r6(d.reduce((a, x) => a + x, 0) / d.length), ci_lower: r6(d[Math.floor(d.length * 0.025)]), ci_upper: r6(d[Math.floor(d.length * 0.975)]),
    ...
    degenerate_note: block >= Math.floor(v.length * 2 / 3) ? "block >= 2T/3: resamples < 1.5 independent blocks; sensitivity only, never verdict-bearing (ISSUE 3)" : undefined }; }
...
// PRIMARY v2 inference: ESS = T(1-rho1)/(1+rho1), clamp >= 3; CI = mean +/- 1.96 sd/sqrt(ESS)
function essInference(v) { const T = v.length; if (T < 5) return null;
  ...
  const rho = acf1(v);
  const ess = Math.max(3, T * (1 - rho) / (1 + rho));
...
// ---------- reproduction gate v2: ordinal + cardinal ----------
...
for (let i = 0; i < Math.min(origins.length, x2.per_origin_rows.length); i++) {
  ...
  if (a.as_of !== b.as_of) gateErrors.push(`origin ${i}: as_of mismatch`);
  if (a.n !== b.n) gateErrors.push(`${a.as_of}: n ${a.n} != ${b.n}`);
  if (Math.abs(a.ic_vp - b.ic_vp) > 1e-6) gateErrors.push(`${a.as_of}: ic_vp mismatch`);
  if (Math.abs(a.ic_bp - b.ic_bp) > 1e-6) gateErrors.push(`${a.as_of}: ic_bp mismatch`);
  if (a.complete !== b.complete) gateErrors.push(`${a.as_of}: complete flag mismatch`); }
// cardinal: tr checksum (rounded 1e-9) + rf0 checksum, recorded in artifact
const trChecksum = sha256str(origins.map(o => o.members.map(m => Math.round(m.ret * 1e9)).join(",")).join("|"));
const rfChecksum = sha256str(origins.map(o => o.rf0).join(","));
const symbolSets = origins.map(o => o.members.map(m => m.symbol).join(","));
const symbolSetChecksum = sha256str(symbolSets.join("|"));
if (gateErrors.length) {
  console.error("ABORT: reproduction gate v2 failed:\n" + gateErrors.join("\n"));
  fs.writeFileSync(path.join(ROOT, "data/computed/feno-rim-recovery/R0_ADJUDICATION_V2.json"),
    JSON.stringify({ phase: "R0", status: "ABORTED_REPRODUCTION_GATE", gate_errors: gateErrors, generated_at: new Date().toISOString() }, null, 2) + "\n");
  process.exit(4); }
console.log("reproduction gate v2 PASSED:", origins.length, "origins ordinal match; tr checksum", trChecksum.slice(0, 12), "rf checksum", rfChecksum.slice(0, 12));
...
// ---------- R0-B partial correlation (primary) + regressor-only (sensitivity) ----------
function r0b(set, label) {
  ...
  return { set: label,
    ...
    sensitivity_regressor_only: agg(regOnly, "resid_ic", "sp(resid_vp, ret) - v1 statistic") }; }
...
// ---------- R0-D stratified permutation + combined mean-S test ----------
function r0d(set, label, seedBase) {
  ...
  set.forEach((o, idx) => {
    ...
    if (Sobs2 === null) { per.push({ as_of: o.as_of, S: null, p_one_sided: null }); return; }
    ...
    for (let r = 0; r < B; r++) {
      ...
      if (S !== null && S >= Sobs2) ge++; }
    per.push({ as_of: o.as_of, S: r6(Sobs2), p_one_sided: r6((ge + 1) / (B + 1)), reps: B }); });
...
function verdict() {
  ...
  return { label, disagreement_between_sets: disagreement,
    zero_unreachable: { declared: true, band_b2: 0.01, min_achievable_ci_halfwidth_b2: { all: eALL.mde, wc: eWC.mde },
      note: "ISSUE 4: at these ESS values the band is several times narrower than the smallest achievable CI; ZERO cannot be declared in practice. INCONCLUSIVE means neither increment nor equivalence is affirmed - the design cannot produce affirmative retirement evidence at n=18/34." },
    effective_n: { all_origins_ess_b2: eALL.ess, window_complete_ess_b2: eWC.ess, all_origins_ess_D: A_all.primary.ess_ci.ess, window_complete_ess_D: A_wc.primary.ess_ci.ess },
    regime_span: { all_origins: [origins[0].as_of, origins[origins.length - 1].as_of],
      ...
      note: "window_complete is the contiguous 2019-03..2023-06 block; return windows span 2019-2026 (ISSUE 11)" },
    survivorship_bound: "Current Dow 30 fixed list applied to all origins from 2015; NVDA and SHW joined the index only in 2024. Bias common to both arms (within-basket contrast valid) but bounds external validity (ISSUE 12)." }; }

const v = verdict();
const firmPanel = { schema_version: "feno_rim_recovery_r0_firm_panel.v2", criteria_sha256: FROZEN_SHA,
  gate_checksums: { tr_sha256: trChecksum, rf0_sha256: rfChecksum, symbol_sets_sha256: symbolSetChecksum },
...
fs.writeFileSync(path.join(ROOT, "data/computed/feno-rim-recovery/r0-firm-panel-v2.json"), JSON.stringify(firmPanel, null, 1) + "\n");
const adj = { schema_version: "feno_rim_recovery_r0_adjudication.v2", phase: "R0", research_only: true,
  criteria_sha256: FROZEN_SHA, supersedes: "R0_ADJUDICATION.json (v1 run authority discarded per option (b); file preserved)",
  reproduction_gate_v2: { ordinal: "PASSED (34 origins match X2 within 1e-6)", cardinal_checksums: { tr_sha256: trChecksum, rf0_sha256: rfChecksum, symbol_sets_sha256: symbolSetChecksum } },
  inference_contract: "PRIMARY: ESS-adjusted CI (ESS=T(1-rho1)/(1+rho1), clamp>=3) + Newey-West lag 11. SENSITIVITY: NW lag 2, circular moving-block percentile CIs blocks 4/8/12 with independent-block counts (block-12 on T=18 degenerate, non-verdict-bearing).",
  ...
  verdict: v,
  generated_at: new Date().toISOString() };
fs.writeFileSync(path.join(ROOT, "data/computed/feno-rim-recovery/R0_ADJUDICATION_V2.json"), JSON.stringify(adj, null, 2) + "\n");

console.log("\n== R0 v2 SUMMARY (primary: ESS CI + NW lag11) ==");
const fmt = (e, nw) => `ess ${e.ess} CI[${e.ci_lower},${e.ci_upper}] NW11 t=${nw.t} p=${nw.p}`;
...
console.log("B all partial:", pfmt(B_all.primary_partial_correlation));
console.log("B wc  partial:", pfmt(B_wc.primary_partial_correlation));
console.log("B wc  regOnly:", pfmt(B_wc.sensitivity_regressor_only));
...
console.log("C2 wc  b2:", C_wc.model2.mean_b2, fmt(C_wc.model2.primary.ess_ci, C_wc.model2.primary.nw_lag11), "VIFratio", C_wc.variance_inflation_m2_vs_m1.mean_se_ratio);
...
function pfmt(x) { return `${x.mean} ess ${x.primary.ess_ci.ess} CI[${x.primary.ess_ci.ci_lower},${x.primary.ess_ci.ci_upper}] NW11 p=${x.primary.nw_lag11.p}`; }
