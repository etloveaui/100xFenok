...
import {
  ...
  RIM_DISCOUNT_SENSITIVITY,
  RIM_EXPLICIT_YEARS,
  RIM_MARKET_RATES,
  RIM_TERMINAL_GROWTH,
  RIM_TERMINAL_ROE_CAP,
...
{
  const g0 = computeFenokRimRow({ px: 2000, pbr: 2, roe: 0.2, discountRate: 0.1, retention: 0.5, terminalGrowth: 0, key: "t", name: "T", date: "d" });
  const g25 = computeFenokRimRow({ px: 2000, pbr: 2, roe: 0.2, discountRate: 0.1, retention: 0.5, terminalGrowth: 0.025, key: "t", name: "T", date: "d" });
  assert.ok(g25.fair_value > g0.fair_value);
  const bad = computeFenokRimRow({ px: 2000, pbr: 2, roe: 0.2, discountRate: 0.02, retention: 0.5, terminalGrowth: 0.025, key: "t", name: "T", date: "d" });
  assert.equal(bad.status, "excluded");
}

// ROE below r produces fair value below book — negative spread reduces.
{
  const row = computeFenokRimRow({ px: 900, pbr: 1.0, roe: 0.05, discountRate: 0.1, retention: 0.5, key: "t", name: "T", date: "d" });
  assert.ok(row.fair_value < row.book_value);
}

// Missing inputs exclude with reasons; no fabricated numbers.
{
  const row = computeFenokRimRow({ px: 100, pbr: null, roe: 0.2, discountRate: 0.1, retention: 0.5, key: "t", name: "T", date: "d" });
  assert.equal(row.status, "excluded");
  assert.match(row.reason, /px_to_book_ratio/);
  const zero = computeFenokRimRow({ px: 100, pbr: 0, roe: 0.2, discountRate: 0.1, retention: 0.5, key: "t", name: "T", date: "d" });
  assert.equal(zero.status, "excluded");
  const badRet = computeFenokRimRow({ px: 100, pbr: 1, roe: 0.2, discountRate: 0.1, retention: 1.2, key: "t", name: "T", date: "d" });
...
// computation, and both values plus the source stay visible in the row.
{
  const row = computeFenokRimRow({
    ...
    roeOverride: { value: 0.2, source: "source_analyst_stated_3y_average_roe" },
    key: "t", name: "T", date: "d",
  });
  ...
  assert.equal(row.roe_override_source, "source_analyst_stated_3y_average_roe");
}
...
{
  const constant = computeFenokRimRow({
    px: 2000, pbr: 2.0, roe: 0.2, discountRate: 0.1, retention: 0.5,
    terminalGrowth: 0, terminalRoeCap: 0.99, key: "t", name: "T", date: "d",
  });
  const fading = computeFenokRimRow({
    px: 2000, pbr: 2.0, roe: 0.2, discountRate: 0.1, retention: 0.5,
...
// source sheets cell: hand-computed reference. B0 = 1000, path [0.2, 0.2, 0.2],
...
// are FY3 x haircut +-0.5%p, and a higher haircut (Likely) beats Worst.
{
  ...
  const worst = computeSheetCase({ ...base, haircut: 0.735 });
  const likely = computeSheetCase({ ...base, haircut: 0.81 });
...
assert.deepEqual(RIM_MARKET_RATES, { us: 0.08, kr: 0.0971 });
assert.equal(RIM_CALIBRATION_ANCHORS.length >= 3, true, "anchors exist for sp500/nasdaq100/kospi");
assert.equal(RIM_DISCOUNT_SENSITIVITY[0], 0.08, "headline rate is the source-calibrated 8%");
assert.ok(RIM_DISCOUNT_SENSITIVITY.includes(0.0971), "house ERP-derived rate stays visible");
assert.equal(RIM_EXPLICIT_YEARS, 3);
assert.equal(RIM_TERMINAL_GROWTH, 0.025);
assert.equal(RIM_TERMINAL_ROE_CAP, null, "no global cap; speaker overrides carry the correction");
