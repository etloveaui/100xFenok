...
export type GeminiLiveTransportMetrics = {
...
export type GeminiLiveTransportStartOptions = {
  resumedFromConversationId?: string | null;
};

export type GeminiLiveTransportRequest<
  TSettings extends object,
  TSession extends GeminiLiveTransportSession,
  TRequestContext extends object = Record<never, never>,
> = (input: {
  settings: TSettings;
  resumedFromConversationId?: string | null;
} & TRequestContext) => Promise<TSession>;

export type GeminiLiveTransportOptions<
  TSettings extends object,
  TSession extends GeminiLiveTransportSession,
  TRequestContext extends object = Record<never, never>,
> = {
  settings: TSettings;
  requestSession: GeminiLiveTransportRequest<TSettings, TSession, TRequestContext>;
  /**
   * Stable product-owned request values read at token-mint time. Keeping this
   * as a callback avoids capturing a stale session identity across reconnects.
   */
  getSessionRequestContext?: () => TRequestContext;
...
const EMPTY_METRICS: GeminiLiveTransportMetrics = {
...
export function useGeminiLiveTransport<
  TSettings extends object,
  TSession extends GeminiLiveTransportSession,
  TRequestContext extends object = Record<never, never>,
>({
  ...
  const [metrics, setMetrics] = useState<GeminiLiveTransportMetrics>(EMPTY_METRICS);
  const socketRef = useRef<WebSocket | null>(null);
  const activeSessionRef = useRef<TSession | null>(null);
  const startMsRef = useRef<number | null>(null);
  const firstResponseSeenRef = useRef(false);
  const turnResponseStartedMsRef = useRef<number | null>(null);
  const turnResponseMeasuredRef = useRef(false);
  ...
  const prewarmedResumeSessionRef = useRef<TSession | null>(null);
  const prewarmResumePromiseRef = useRef<Promise<TSession | null> | null>(null);
  const prewarmRequestTokenRef = useRef<symbol | null>(null);
  ...
  const failFatal = useCallback((message: string, eventType: string) => {
    setStatus("error");
    setMetrics((current) => ({ ...current, lastError: message }));
    emitEvent(eventType, message);
    if (fatalErrorNotifiedRef.current) return;
    fatalErrorNotifiedRef.current = true;
    onFatalError?.(message);
  }, [emitEvent, onFatalError]);
  ...
  const prewarmResumeSession = useCallback((reason: string) => {
    ...
    const requestToken = Symbol("gemini-live-resume-prewarm");
    prewarmRequestTokenRef.current = requestToken;
    ...
    prewarmResumePromiseRef.current = requestSession({
      settings,
      resumedFromConversationId: liveSession.conversationId,
      ...(getSessionRequestContext?.() ?? {} as TRequestContext),
    }).then((payload) => {
      const stillCurrent = prewarmRequestTokenRef.current === requestToken
      ...
      if (prewarmRequestTokenRef.current === requestToken) {
        prewarmRequestTokenRef.current = null;
  ...
  const stop = useCallback((finalStatus: MonaVnextLiveStatus = "stopped") => {
    ...
    turnResponseStartedMsRef.current = null;
    turnResponseMeasuredRef.current = false;
  ...
  const handleServerMessage = useCallback((
    ...
    if (
      serverContent.inputTranscription?.text?.trim()
      && turnResponseStartedMsRef.current === null
    ) {
      turnResponseStartedMsRef.current = performance.now();
      turnResponseMeasuredRef.current = false;
    }
    ...
    if (playedAudio && !firstResponseSeenRef.current) {
      ...
      emitEvent("first_model_audio_ms", "Session-start to first model audio latency measured.", {
    ...
    if (
      playedAudio
      && turnResponseStartedMsRef.current !== null
      && !turnResponseMeasuredRef.current
    ) {
      turnResponseMeasuredRef.current = true;
      const responseLatencyMs = Math.round(
        performance.now() - turnResponseStartedMsRef.current,
      );
      setMetrics((current) => ({
        ...current,
        lastResponseLatencyMs: responseLatencyMs,
        responseLatencySamplesMs: [
          ...current.responseLatencySamplesMs,
          responseLatencyMs,
        ],
      }));
      emitEvent(
        "turn_response_audio_ms",
        "Learner input to first model audio latency measured.",
  ...
  const openSocket = useCallback((
    ...
    socket.onopen = () => {
      ...
      try {
        ...
        failFatal(message, "socket-setup-send-error");
        return;
      }
      emitEvent(
    ...
    socket.onmessage = (event) => {
      void (async () => {
        try {
          ...
          failFatal(message, "server-message-error");
        }
      })();
    };
    ...
    socket.onerror = () => {
      ...
      failFatal(message, "socket-error");
    };
    ...
    socket.onclose = (event) => {
      ...
      if (event.code !== 1000) {
        ...
        failFatal(message, "socket-close-error");
      } else {
        failFatal("GEMINI_SOCKET_CLOSED_1000", "socket-close-unexpected");
      }
    };

    return socket;
  }, [audioInput, audioOutput, emitEvent, failFatal, failRecovery, handleServerMessage]);
  ...
  const start = useCallback(async (
    options: GeminiLiveTransportStartOptions = {},
  ) => {
    ...
    try {
      ...
      const liveSession = await requestSession({
        settings,
      ...
      if (hadStartedSession && !fatalErrorNotifiedRef.current) {
        fatalErrorNotifiedRef.current = true;
        onFatalError?.(message);
      }
    }
  }, [audioInput, audioOutput, emitEvent, getSessionRequestContext, onFatalError, onSessionReady, openSocket, requestSession, settings, status, stop]);
