...
export default async function PostsPage({ searchParams }: PageProps) {
  ...
  const landing = (
    <div
      data-posts-surface
      data-posts-route-owner="analysis-archive"
    ...
    >
      <div className="container mx-auto p-3 sm:p-4 md:p-8">
        ...
        <section
          data-posts-boundary
          className="mb-8 rounded-lg border border-slate-200 bg-white p-4 shadow-sm sm:p-5"
        >
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl">
              ...
              <p className="mt-2 text-sm leading-relaxed text-slate-600">
                ...
                Alpha Scout와 Daily Wrap으로 분리합니다.
              </p>
            </div>
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-3">
            {POSTS_OWNER_LINKS.map((link) => (
              <Link
                key={link.key}
                href={link.href}
                data-posts-owner-link={link.key}
                className="inline-flex min-h-11 items-center justify-center rounded-lg border border-slate-200 bg-white px-3 text-sm font-bold text-slate-700 shadow-sm transition hover:border-blue-300 hover:text-blue-700"
              >
                {link.label}
              </Link>
