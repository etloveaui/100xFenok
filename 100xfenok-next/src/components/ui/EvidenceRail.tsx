...
export function EvidenceRail({
  ...
  if (variant === "chip") {
    ...
    return (
      <button onClick={onEvidence} className={`evidence-rail-action ${chipClass} focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-interactive`} aria-label={`증거: ${source} ${asOf}`}>
        {chipInner}
      </button>
    );
  }
    ...
    <div className={className}>
      ...
      >
        ...
        {showRetry && (
          <button
            onClick={onRetry}
            className={`evidence-rail-action ${hasDrawer || onEvidence ? "" : "ml-auto "}shrink-0 font-semibold text-[#1B73D3] hover:text-[#155fae] transition-colors duration-150 focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-interactive max-md:min-h-[44px] max-md:inline-flex max-md:items-center`}
          >
            지금 재시도
          </button>
        )}
        {(hasDrawer || onEvidence) && (
          <button onClick={handleEvidence} aria-expanded={hasDrawer ? drawerOpen : undefined} aria-controls={hasDrawer ? drawerId : undefined} className="evidence-rail-action ml-auto shrink-0 font-semibold text-[#1B73D3] hover:text-[#155fae] transition-colors duration-150 focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-interactive max-md:min-h-[44px] max-md:inline-flex max-md:items-center">
            증거 보기
          </button>
        )}
      </div>
