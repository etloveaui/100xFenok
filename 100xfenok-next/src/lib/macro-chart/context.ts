...
export const MACRO_CONTEXTS: Record<MacroContextId, MacroWorkbenchContext> = {
  "risk-liquidity": {
    ...
    chartHref: ROUTES.macroChartQuery("macro=risk-liquidity&series=sp500,vix,tga,DGS10,HY_spread,M2SL&transform=rebase100,raw,rebase100,raw,raw,yoy&range=10Y&hidden=vix&axis=vix:right,DGS10:right,HY_spread:right&formula=ratio:sp500:DGS10"),
    screenerHref: `${ROUTES.screener}?macro=risk-liquidity&preset=connected&connection=indexMembership`,
    etfHref: `${ROUTES.etfs}?macro=risk-liquidity&type=inverse`,
    stockHref: `${ROUTES.stock("NVDA")}?macro=risk-liquidity`,
    stockSymbol: "NVDA",
    stockLabel: "AI 대형주",
  ...
  "bank-credit": {
    ...
    chartHref: ROUTES.macroChartQuery("macro=bank-credit&series=bank_credit,deposits,fdic_tier1,HY_spread,DGS10&transform=yoy,yoy,raw,raw,raw&range=10Y&axis=fdic_tier1:right,HY_spread:right,DGS10:right&formula=spread:bank_credit:deposits"),
    screenerHref: `${ROUTES.screener}?macro=bank-credit&preset=value&connection=filings`,
    etfHref: `${ROUTES.etfs}?macro=bank-credit&asset=Bond&fee=low`,
    stockHref: `${ROUTES.stock("JPM")}?macro=bank-credit`,
    stockSymbol: "JPM",
    stockLabel: "은행 대표주",
    screenerPreset: "가치",
    insightBullets: [
  ...
  activity: {
    ...
    chartHref: ROUTES.macroChartQuery("macro=activity&preset=activity&range=MAX"),
    screenerHref: `${ROUTES.screener}?macro=activity&preset=estimate&action=value_momentum`,
    etfHref: `${ROUTES.etfs}?macro=activity&type=leveraged`,
    stockHref: `${ROUTES.stock("AAPL")}?macro=activity`,
    stockSymbol: "AAPL",
  ...
  "crypto-liquidity": {
    ...
    chartHref: ROUTES.macroChartQuery("macro=crypto-liquidity&series=stablecoins,nasdaq,sp500,crypto_fear_greed,vix&transform=rebase100,rebase100,rebase100,raw,raw&range=5Y&hidden=vix&axis=crypto_fear_greed:right,vix:right&formula=ratio:nasdaq:stablecoins"),
    screenerHref: `${ROUTES.screener}?macro=crypto-liquidity&preset=momentum&action=momentum`,
    etfHref: `${ROUTES.etfs}?macro=crypto-liquidity&digital=1`,
    stockHref: `${ROUTES.stock("COIN")}?macro=crypto-liquidity`,
    stockSymbol: "COIN",
    stockLabel: "크립토 민감주",
    screenerPreset: "모멘텀",
    insightBullets: [
