...
import type { FenokEtfSignalsSummaryReadResult } from "./data-loader";
...
type EtfSignalSummaryPayload = {
  ...
  signal_definitions?: Record<string, unknown>;
...
export function buildEtfSignalRouteResponse(
  result: FenokEtfSignalsSummaryReadResult,
  normalizedTicker: string,
) {
  if (result.kind !== "ok") {
    return NextResponse.json(
      { error: "FENOK_ETF_SIGNAL_UNAVAILABLE", ticker: normalizedTicker, reason: result.reason },
      { status: 503, headers: { "Cache-Control": "no-store" } },
    );
  }
  const payload = result.payload as unknown as EtfSignalSummaryPayload;
  if (!Array.isArray(payload.rows) || payload.rows.some((rawRow) => {
    const row = normalizeEtfSignalRow(payload, rawRow);
    return typeof row?.ticker !== "string" || !cleanTicker(row.ticker);
  })) {
    return NextResponse.json(
      { error: "FENOK_ETF_SIGNAL_UNAVAILABLE", ticker: normalizedTicker, reason: "FENOK_ETF_SIGNAL_INVALID_PAYLOAD" },
      { status: 503, headers: { "Cache-Control": "no-store" } },
    );
  }
  ...
  if (!row) {
    return NextResponse.json(
      { error: "FENOK_ETF_SIGNAL_NOT_FOUND", ticker: normalizedTicker },
  ...
  return NextResponse.json(
    {
      ...
      signal_definitions: payload.signal_definitions ?? null,
