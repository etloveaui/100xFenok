# Market Radar - DEV.md

> Market Radar 차트 개발 메모
> 생성일: 2025-12-31
> VIX update note: `scripts/vix.gs` is deprecated as of 2026-06-22. Runtime VIX
> updates are owned by `../../scripts/fetch-sentiment.mjs` via
> `../../.github/workflows/fetch-sentiment.yml`.

---

## Purpose

Market Sentiment + Market Indices 차트 시각화. Signal Score Timeline, MA 오버레이, 유연한 기간 선택 지원.

---

## Folder Structure

```
admin/market-radar/
├── DEV.md           ← 이 파일
├── charts/          ← admin chart index/test utilities only
└── scripts/         ← deprecated Apps Script backups
    ├── README.md
    ├── vix.gs
    ├── move.gs
    └── indices.gs   ← SP500/NASDAQ (예정)
```

---

## Charts Overview

> **Current owner (2026-07-08)**: The 11 sentiment chart HTML files now live
> under `../../tools/macro-monitor/details/sentiment-signal/charts/`. The
> `admin/market-radar/charts/` folder no longer owns those chart files.
> Indices chart notes below are historical.

### 1. Sentiment 차트 (5개) - Signal Score Timeline

| 파일 | 지표 | 데이터 |
|------|------|--------|
| `../../tools/macro-monitor/details/sentiment-signal/charts/chart-aaii.html` | AAII 투자자 심리 | `aaii.json` |
| `../../tools/macro-monitor/details/sentiment-signal/charts/chart-cftc.html` | CFTC 포지션 | `cftc-sp500.json` |
| `../../tools/macro-monitor/details/sentiment-signal/charts/chart-cnn-fg.html` | CNN Fear & Greed | `cnn-fg.json` |
| `../../tools/macro-monitor/details/sentiment-signal/charts/chart-crypto-fg.html` | Crypto Fear & Greed | `crypto-fg.json` |
| `../../tools/macro-monitor/details/sentiment-signal/charts/chart-vix-move.html` | VIX + MOVE 듀얼 | `vix.json`, `move.json` |

### 2. CNN Sub-indicator 차트 (6개) - 원시 측정값

| 파일 | 지표 | 표시방식 |
|------|------|---------|
| `../../tools/macro-monitor/details/sentiment-signal/charts/chart-cnn-momentum.html` | S&P vs 125-day MA | 듀얼 라인 |
| `../../tools/macro-monitor/details/sentiment-signal/charts/chart-cnn-strength.html` | 52주 고/저점 비율 | 제로라인 기준 |
| `../../tools/macro-monitor/details/sentiment-signal/charts/chart-cnn-breadth.html` | McClellan VSI | 백분위 임계값 |
| `../../tools/macro-monitor/details/sentiment-signal/charts/chart-cnn-put-call.html` | Put/Call 비율 | 0.7/1.0 임계존 |
| `../../tools/macro-monitor/details/sentiment-signal/charts/chart-cnn-junk-bond.html` | 정크본드 스프레드 | 동적 임계값 |
| `../../tools/macro-monitor/details/sentiment-signal/charts/chart-cnn-safe-haven.html` | 주식/국채 수익률 | 제로라인 기준 |

### 3. Market Indices 차트 (2개) - MA 오버레이

> **⚠️ Relocated (2026-01-25)**: Moved to Valuation Lab for data-tool mapping consistency
> - `../../valuation-lab/expansion/indices-sp500.html`
> - `../../valuation-lab/expansion/indices-nasdaq.html`

| 파일 | 지표 | 데이터 |
|------|------|--------|
| ~~`chart-sp500.html`~~ | S&P 500 | `sp500.json` (11,594개) |
| ~~`chart-nasdaq.html`~~ | NASDAQ | `nasdaq.json` (11,594개) |

---

## Current Implementation

### S&P 500 차트 (2025-12-31)

**기능**:
| 항목 | 구현 |
|------|------|
| MA 토글 | 20D (🩵 cyan), 50D (🟣 purple), 200D (🟠 orange) |
| 기간 선택 | 스마트 드롭다운 (Quick/Standard/Long-term/Year/Custom) |
| YTD | ✅ 지원 |
| 특정 연도 | ✅ 최근 10년 |
| 커스텀 범위 | ✅ 시작~종료일 직접 선택 |
| Recession | 음영 annotation |
| 모바일 | 하단 시트 스타일 |

**색상 테마**: Blue (#3b82f6)

### NASDAQ 차트 (2025-12-31)

- S&P 500과 동일 기능
- **색상 테마**: Green (#10b981)

---

## Data Sources

### Indices
| 파일 | 레코드 | 기간 | 소스 |
|------|--------|------|------|
| `data/indices/sp500.json` | 11,594+ | 1980-01-02 ~ | GOOGLEFINANCE |
| `data/indices/nasdaq.json` | 11,594+ | 1980-01-02 ~ | GOOGLEFINANCE |

### Sentiment
| 파일 | 레코드 | 기간 | 소스 |
|------|--------|------|------|
| `data/sentiment/vix.json` | 9,091+ | 1990-01-02 ~ | `fetch-sentiment.mjs` (Yahoo ^VIX) |
| `data/sentiment/move.json` | 1,249+ | 2020-12-29 ~ | Yahoo Proxy |

**필드**: `{ date, value }`

---

## Data Collection (Apps Script)

> 코드 백업: `scripts/` 폴더 참조

| 수집기 | 트리거 | 방식 |
|--------|--------|------|
| SP500/NASDAQ | 06:00, 09:00 | GOOGLEFINANCE → merge → GitHub |
| VIX | GitHub Actions 평일 after-close | `fetch-sentiment.mjs` → merge → repo SSOT + Next public mirror |
| MOVE | 07:29 | Yahoo Proxy → merge → GitHub |

**⚠️ 필수**: legacy Apps Script backups are preserved for reference only;
runtime sentiment updates are owned by the GitHub Actions / Node pipeline noted
at the top of this file.

---

## UI/UX

**스마트 드롭다운 기간 선택**:
| 카테고리 | 옵션 |
|----------|------|
| ⚡ Quick | 1W, 2W, 1M, 3M |
| 📈 Standard | 6M, YTD, 1Y, 2Y |
| 📊 Long-term | 3Y, 5Y, 10Y, 20Y |
| 📅 Year | 2025, 2024, ... (최근 10년) |
| 🗓️ Custom | 시작 ~ 종료 날짜 |

**MA Chip 토글**:
- 20D: 기본 OFF (🩵)
- 50D: 기본 ON (🟣)
- 200D: 기본 ON (🟠)

**반응형**:
- Mobile: 하단 시트 + 3열 그리드
- Desktop: 플로팅 패널 + 4열 그리드

---

## Phase Checklist

### Phase 1: 기본 구현 ✅ (2025-12-31)
- [x] S&P 500 차트 구현
- [x] NASDAQ 차트 구현
- [x] 20/50/200 MA 토글
- [x] 스마트 드롭다운 기간 선택
- [x] YTD, 특정 연도, 커스텀 범위
- [x] 모바일 최적화
- [x] `/admin/market-radar` owner route 등록 (`admin/index.html` shell retired 2026-07-08)

### Phase 2: 확장 (진행 중)
- [x] 데이터 자동 수집 파이프라인 ✅ (#77, DEC-076)
  - Apps Script + GOOGLEFINANCE + GitHub API
  - 병합 방식 (기존 데이터 유지)
  - 트리거: 매일 06:00 + 09:00
- [ ] 더 많은 지수 추가 (DJI, Russell 등)
- [ ] 비교 차트 기능

---

## Known Issues

- (현재 없음)

---

## Change Log

> 상세 이력: `CookBook/docs/CHANGELOG.md` 참조

| 날짜 | 변경 |
|------|------|
| 04-14 | Added `embed=1` shell mode to `index.html` so iframe bridge hides legacy header/footer under Next.js outer shell (#269) |
| 01-02 | **🔴 Sentiment Dashboard 전면 재설계 (#98)** |
| 01-02 | 콤보시그널 10개 구현 (매수 7 + 경고 3) |
| 01-02 | JSON 데이터 연동: `data/sentiment/` → `loadMarketData()` |
| 01-02 | 11개 지표 설명 풍부화 (HTML 태그, 임계값 테이블, 백테스트 데이터) |
| 01-02 | Put/Call Ratio 학술 연구 기반 추가 (Billingsley & Chance 1988) |
| 01-02 | 페이지 제목 재설계: 100x Combo Signals → Market Sentiment (DEC-084) |
| 01-02 | 차트 하단 설명 삭제 (9개 파일) → 탭 통합 |
| 01-02 | innerHTML 수정: textContent → innerHTML (HTML 렌더링) |
| 01-01 | scripts/ 폴더 생성 (Apps Script 백업) |
| 01-01 | VIX/MOVE merge 패턴 적용 + 테스트 완료 |
| 12-31 | 스마트 드롭다운 기간 선택기 구현 |
| 12-31 | YTD, 연도별, 커스텀 범위 지원 |
| 12-31 | 20/50/200 MA 토글 구현 |
| 12-31 | 모던 UI 리디자인 (Inter, Chip, Segment) |
