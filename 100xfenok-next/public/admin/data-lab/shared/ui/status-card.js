/**
 * StatusCard - Reusable data source status component
 *
...
const StatusCard = (function() {
      ...
      <div class="bg-white rounded-xl p-5 shadow hover:shadow-md transition-shadow"
        ...
        <div class="mt-3 pt-3 border-t border-gray-100 flex justify-between items-center">
          ...
          <button class="text-xs text-${color}-600 hover:text-${color}-800 font-medium"
                  onclick="DataLabUI.showFolderDetails('${folderName}')">
            Details →
          </button>
        </div>
      </div>
   ...
   * @returns {string} HTML string
   */
  function renderAll(foldersWithSchemas, freshnessResults) {
    return Object.entries(foldersWithSchemas)
      .map(([name, data]) => render(name, data.config, freshnessResults[name] || {}))
      .join('');
  }
  ...
  function renderError(message) {
    return `
      <div class="col-span-full bg-red-50 border border-red-200 rounded-xl p-6 text-center">
        ...
        <button onclick="DataLabUI.refresh()" class="mt-4 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 text-sm">
          Retry
        </button>
      </div>
    `;
