...
// 5w. watchlist — star AAPL then verify explore strip
const star = page.getByRole("button", { name: /관심종목 추가/ });
if (await star.count()) {
  await star.first().click();
  await page.waitForTimeout(600);
  await shot("05w-stock-aapl-starred");
  await page.goto(`${BASE}/explore`, { waitUntil: "load" });
  await page.waitForTimeout(3000);
  await shot("05x-explore-watchlist-strip");
} else {
  console.log("!! watch star NOT FOUND");
}
