...
export function validateRenderManifest(manifest, reader = read) {
  ...
  assert.equal(manifest.site_count, 94, "builder-derived roster after the light-system CSS witness migrations retired six net render sites (100 to 94)");
  assert.equal(manifest.consumer_count, 95, "builder-derived consumer count remains exactly one above the render-site count");
  ...
  assert.deepEqual(
    roles,
    { body_text: 87, non_text: 4, mixed_text_non_text: 1, inactive_control: 2 },
    "text, non-text, or inactive-control classification drifted from the builder-derived roster (body_text 93 to 87)",
  );
