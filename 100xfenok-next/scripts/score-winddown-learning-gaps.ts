...
async function main() {
  await check("finalized signed correction reaches practice; forgery and cancellation do not", async () => {
    ...
    assert.equal(await verifyWindDownCoachFeedbacks({ productSessionId: binding.productSessionId, turns: [{ ...attached, coachFeedback: { ...feedback, now: "I will go home" } }] }), false);
    const outcome = summarizeWindDownLiveTalk({ turns: [attached], startedAtIso: null, stoppedAtIso: new Date().toISOString() });
    assert.equal(outcome.corrections.length, 1);
    const seeds = extractWindDownVoicePracticeSeeds({ productSessionId: binding.productSessionId, activity: "live-talk", report: { turns: [attached], outcome }, journeyTargets: [] } as Parameters<typeof extractWindDownVoicePracticeSeeds>[0]);
