...
> **Created**: 2026-02-02
> **Updated**: 2026-02-03
> **Status**: ✅ Phase 1-3 Complete + Order Execution + **현재가 연동 (#211)** + **UX 개선 (#216, #219)** + **다중 종목 계산 (#217, #218)** + **로그인 기반 초기화 (v4.29)**
...
const starPercent = 10 - (T / 2);

// Examples:
// T=2  → 별% = 10 - 1 = 9%
// T=10 → 별% = 10 - 5 = 5%  (전후반전 기준)
// T=20 → 별% = 10 - 10 = 0%
// T=40 → 별% = 10 - 20 = -10%
```
...
| SOXL | 1.0 | 9.5% | $53.90 | $54.83 | $54.83 | 현재가+15% |
| BITU | 21.9 | -0.95% | $21.98 | $21.24 | $21.24 | 현재가+15% |
...
| SOXL | 1.0 | 9.5% | 현재가+15% 선택 |
| BITU | 21.9 | -0.95% | 현재가+15% 선택 |
...
**A**: V2.2 방법론 원본에서는 TQQQ/SOXL 모두 `10 - T/2` 사용.
차이점은 **매도비율**만 다름 (TQQQ 10%, SOXL 12%).
기존 `ib-total-guide-calculator.html`의 SOXL 공식은 **틀림**.
